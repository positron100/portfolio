...
export function useThemeToggleController(
   ...
   * the toggle lives at the bottom of the nav menu panel, which is still
   * running its open spring (an under-damped `scaleY` overshoot, plus the
   * menu item's own `y`/`scale`) for ~400ms after it opens.
   * `getBoundingClientRect` folds every ancestor transform into its result,
   * so a tap during that window measured a distorted point ~130px off and the
   * reveal opened from the wrong place — correct on desktop (toggle has no
   * animated ancestor), wrong on mobile.
   *
